        ...
        <div class="modal fade" id="createCampainsModal" tabindex="-1" aria-labelledby="createCampainsModalLabel"
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <form class="from" action="{{ route('admin.campaign.create') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('POST')
                        <div class="modal-body">
                            <div class="row">
                                ...
                                <div class="mb-3 col-md-6">
                                    <label for="create_campaignAmount" class="form-label required">Amount</label>
                                    <input type="number" class="form-control" required id="create_campaignAmount"
                                        name="amount">
                                </div>
                                <div class="mb-3 col-md-6">
                                    <label for="createCampaignGame" class="form-label required">Select a Game</label>
                                    <select class="form-select" name="game_id" id="createCampaignGame" required>
                                        <option value="0" selected disabled>Select a Game</option>
                                        @foreach ($games as $game)
                                            <option value="{{ $game->id }}">{{ $game->title }} ({{ $game->keyword }})
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
            ...
            $('#campaignsTableId').DataTable({
                ...
                columns: [{
                    ...
                    },
                    {
                        render: function(data, type, row) {
                            const btns =
                                `<button type="button" class="btn btn-sm btn-primary" onClick="updateCampaign(${row.id})">Edit</button>`;
                            return btns;
                        },
                        targets: 0,
                        className: 'fit-content'
