...
class PurchaseController extends Controller
{
    ...
    public function index(Request $request){
        if (request()->ajax()) {
            $query = Purchase::where('company_id', Auth::user()->company_id)->with('supplier')->orderBy('created_at', 'desc')->get();
    ...
    public function create(Request $request)
    {
        ...
        try {
            ...
            $curl = curl_init();
            $url = url('/api/log/transection?transaction_type_id=' . $purchasePayment->transaction_type_id
                . '&type=1&amount=' . $purchasePayment->amount . '&ref_id=' . $purchasePayment->id);
            // Set cURL options
