...
const fetchData = async (from, to, supplier_id) => {
    try {
        const response = await axios.get(
            `/report/purchase-payment?type=fetch&start_date=${from}&end_date=${to}&supplier_id=${supplier_id}`
        );
...
const exportToExcel = async () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Purchase Payment Report Data");

    // Create a dynamic file name
    const fileName = `Purchase Payment Report ${formattedDate}.xlsx`;
