...
const fetchData = async (from, to, supplier_id) => {
    console.log(from, to, supplier_id);
    try {
        const response = await axios.get(
            `/report/purchase-return?type=fetch&start_date=${from}&end_date=${to}&supplier_id=${supplier_id}`
        );
...
const exportToExcel = async () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Purchase Report Return Data");

    // Create a dynamic file name
    const fileName = `Purchase Report Return ${formattedDate}.xlsx`;
