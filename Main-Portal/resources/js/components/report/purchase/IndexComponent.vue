...
const fetchData = async (from, to, supplier_id) => {
    ...
    try {
        const response = await axios.get(
            `/report/purchase?type=fetch&start_date=${from}&end_date=${to}&supplier_id=${supplier_id}`
        );
        ...
        if (GET_DATA.length > 0) {
            var formattedData = GET_DATA.map((item, index) => {
                return {
                    ...
                    paid_amount: item.paid_amount.toFixed(2),
            ...
            totalPaidAmount.value = formattedData
                .reduce((sum, item) => sum + parseFloat(item.paid_amount), 0)
                .toFixed(2);
            ...
            const newDATA = [
                {
                    ...
                    paid_amount: totalPaidAmount.value,
