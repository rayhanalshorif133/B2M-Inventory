...
import AddNewCustomer from "./AddNewCustomerModal.vue";
    ...
    setup() {
        ...
        const fetchCustomers = () => {
            axios.get("/customer/fetch").then((response) => {
                ...
                selectedCustomer.value = getdata.value?.sales?.customer_id;
            });
        };
        ...
        const addToProductCustomization = (item) => {
            ...
            if (hasAlreadyAdded) {
                ...
                hasAlreadyAdded.total =
                    (parseFloat(hasAlreadyAdded.sales_rate) -
                ...
                const SET_VALUE = {
                    ...
                    sales_rate: item.sales_rate ? item.sales_rate : 0,
                    qty: 0,
                    discount: 0,
                    total: 0,
                };
        ...
        const removeItem = (id) => {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    axios
                        .delete(`/sales/{id}/delete/?type=sales_details`)
                        .then((result) => {
                            productCustomizations.value =
                                productCustomizations.value.filter(
                                    (item) => item.id != id
                                );
                            Swal.fire({
                                title: "Removed!",
