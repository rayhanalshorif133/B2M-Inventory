...
var salesEditLink = ref("");

const fetchData = () => {
    axios.get("/sales/list?type=fetch").then(function (response) {
            ...
            GET_DATA.map(function (item, index) {
                const btns = `
                    <div class="btn-group">
                        ...
                        <a href="/sales/invoice/${item.id}" type="button" class="btn btn-primary btn-sm print btn-fit">
...
const fetchDetailsPurchaseData = (id) => {
    axios.get(`/sales/fetch/${id}`).then((response) => {
        ...
        const modalElement = document.getElementById("showSalesDetails");
        salesEditLink.value = `/sales/${id}/edit/`;
