                    ...
                    <div class="row">
                        ...
                        <div class="col-12 col-md-3 col-sm-6">
                            ...
                            >
                                ...
                                >
                                    {{ customer.name }}
                                    <!-- Assuming each customer has a name and an id -->
                                </option>
                            </select>
                        </div>
...
const fetchcustomersData = ref(props.customers);
...
const fetchData = async (from, to, customer_id) => {
    try {
        const response = await axios.get(
            `/report/sales-payment?type=fetch&start_date=${from}&end_date=${to}&customer_id=${customer_id}`
        );
        ...
        if (GET_DATA.length > 0) {
            var formattedData = GET_DATA.map((item, index) => {
                return {
                    list: index + 1,
                    invoice_date: item.created_date,
...
const exportToExcel = async () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, "Sales Payment Report Data");

    // Create a dynamic file name
    const fileName = `Sales Payment Report ${formattedDate}.xlsx`;
...
<body>
    <div class="container mt-5">
        <div class="row">
            ...
            <div class="col-md-10">
                <h2 class="text-center">Sales Payment Report</h2>
